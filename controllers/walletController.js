...
// === MANUAL VERIFY TRANSACTION (called from app after browser payment) ===
exports.verifyTransaction = async (req, res) => {
  const { reference } = req.body;
  const userId = req.user?.id;

  console.log("🔍 Manual verify transaction requested", {
    reference,
    userId,
  });
  ...
  try {
    // 1. Verify the transaction directly with Paystack API
    const verification = await axios.get(
      `https://api.paystack.co/transaction/verify/${reference}`,
      {
        headers: {
    ...
    if (data.status !== "success") {
      return res.json({
        success: false,
        message: `Payment status is "${data.status || "unknown"}". Please try again or contact support.`,
      });
    }

    // 2. Extract metadata
    const metadata = data.metadata || {};
    const paymentUserId = metadata.user_id;
    const baseAmount = Number.parseFloat(metadata.base_amount || 0);
    const ref = data.reference;

    if (!paymentUserId || !baseAmount) {
      return res.status(400).json({
        success: false,
    ...
    // 3. Credit the wallet (idempotent - if already processed, it no-ops)
    const result = await processWalletFundingCredit({
    ...
    console.log("✅ Manual verify processed", {
      userId: paymentUserId,
      ref,
      baseAmount,
      credited: result.credited,
    ...
    return res.status(500).json({
      success: false,
      message: "Could not process payment. Please contact support.",
    });
  } catch (err) {
    ...
    if (err.response?.status === 404) {
      return res.json({
        success: false,
        message:
          "Payment reference not found on Paystack. Please ensure you completed the payment.",
      });
    }
